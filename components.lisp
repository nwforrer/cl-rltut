...
(defclass confused-monster (component)
  ((previous-ai :initarg :previous-ai :accessor ai/previous-ai)
   (num-turns :initarg :num-turns :accessor ai/num-turns)))

(defmethod take-turn ((component confused-monster) target map entities)
  (let ((results nil)
        (monster (component/owner component)))
    (cond ((> (ai/num-turns component) 0)
           (let* ((cur-x (entity/x monster))
                  (cur-y (entity/y monster))
                  (random-x (- (random 3) 1))
                  (random-y (- (random 3) 1))
                  (dest-x (+ cur-x random-x))
                  (dest-y (+ cur-y random-y)))
             (if (and (/= cur-x random-x)
                      (/= cur-y random-y)
                      (not (blocking-entity-at entities dest-x dest-y)))
                 (move monster random-x random-y))
             (decf (ai/num-turns component))))
          (t
           (setf (entity/ai monster) (ai/previous-ai component))
           (setf results (list :message (format nil "The ~A is no longer confused!" (entity/name monster))
                               :message-color (blt:red)))))
    results))
